...
public_users.get('/',function (req, res) {
    ...
    return res.status(300).json({message: "Books available in the shop"});
});

// Get book details based on ISBN
public_users.get('/isbn/:isbn',function (req, res) {
    res.send(books[req.params.isbn])
    return res.status(300).json({message: `Information on the book with ISBN ${req.params.isbn}`});
});
...
public_users.get('/author/:author',function (req, res) {
    ...
    return res.status(300).json({message: "Information on the book"});
});
...
public_users.get('/review/:isbn',function (req, res) {
    ...
    return res.status(300).json({message: `Reviews for the book with the ISBN ${req.params.isbn}`});
});
